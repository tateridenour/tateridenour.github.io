...
function scrollCarousel(direction) {
	...
	if (direction == "left") {
		...
		if (transitioning == null) {
			transitioning = document.querySelector(
				".carousel__project:last-child"
			);
...
function updateCircles(n) {
	...
	if (n == undefined) {
		...
		const index = Array.prototype.indexOf.call(
			projects,
			document.querySelector(".carousel__transitioning")
		);
		document
			.querySelector(`.circle:nth-child(${index + 1})`)
			.classList.add("active");
	} else {
		document
			.querySelector(`.circle:nth-child(${n})`)
			.classList.add("active");
	}
}
...
function toggleGrid() {
	if (grid.style.opacity === "1") {
		...
		[...document.querySelectorAll(".grid__works__square")].forEach(
			(square) => {
...
function setCarouselTo(n) {
	const target = document.querySelector(
		`#carousel .carousel__project:nth-child(${n})`
	);
